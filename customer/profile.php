...
if(isset($_POST['apply_voucher']) && isset($_POST['voucher_code'])){
    ...
    $today = date('Y-m-d');
    $check = mysqli_query($conn, "SELECT * FROM vouchers WHERE code = '$voucher_code' AND status = 'active' AND (start_date IS NULL OR start_date <= '$today') AND (end_date IS NULL OR end_date >= '$today')");
...
if(isset($_POST['edit_address']) && isset($_POST['address_id'])){
    $editing_id = intval($_POST['address_id']);
}
...
<div>
    <div class="myaccount-content">
            ...
            <div class="row">
                ...
                <?php else: ?>
                    <form method="post">
                        ...
                        ?>
                                <div class="list-group-item">
                                    <?php if(!empty($editing_id) && $editing_id === $aid):
                                        // fetch fresh data for this address
                                        ...
                                        <div class="d-flex justify-content-between align-items-start">
                                            ...
                                            <div class="btn-group btn-group-sm" role="group">
                                                <form method="post" style="display:inline">
                                                    ...
                                                    <button type="submit" name="edit_address" class="btn btn-outline-secondary">Sửa</button>
                                                </form>
                                                <form method="post" style="display:inline" onsubmit="return confirm('Bạn có chắc muốn xóa địa chỉ này?');">
                                                    <input type="hidden" name="address_id" value="<?php echo $aid; ?>">
                                                    <button type="submit" name="delete_address" class="btn btn-outline-danger">Xóa</button>
                                                </form>
                                            </div>
                                        </div>
                                    <?php endif; ?>
                        ...
                        <div class="mt-2">
                            <button type="submit" name="set_default_address" class="btn btn-primary btn-sm">Chọn làm địa chỉ mặc định</button>
                        </div>
                    </form>
                <?php endif; ?>
                </div>
        ...
        <div class="mt-4">
            ...
            <?php
                $today = date('Y-m-d');
                $get_vouchers = "SELECT * FROM vouchers WHERE status = 'active' AND (start_date IS NULL OR start_date <= '$today') AND (end_date IS NULL OR end_date >= '$today')";
                $run_vouchers = mysqli_query($conn, $get_vouchers);
                if(mysqli_num_rows($run_vouchers) == 0){
                    echo "<p>Hiện không có mã giảm giá khả dụng.</p>";
                } else {
                    echo "<div class='row'>";
                    while($v = mysqli_fetch_assoc($run_vouchers)){
                        $code = htmlspecialchars($v['code']);
                        $percent = intval($v['discount_percent']);
                        $amount = intval($v['discount_amount']);
                        $min = intval($v['min_order']);
                        $max = intval($v['max_discount']);
                        echo "<div class='col-md-6 mb-2'>";
                        echo "<div class='card p-2'>";
                        echo "<div class='d-flex justify-content-between align-items-start'>";
                        echo "<div><strong>$code</strong><div class='small text-muted'>";
                        if($percent) echo "Giảm $percent%"; elseif($amount) echo "Giảm " . number_format($amount) . "đ";
                        if($min) echo " • Đơn tối thiểu: " . number_format($min) . "đ";
                        echo "</div></div>";
                        echo "<form method='post' class='mb-0'>";
                        echo "<input type='hidden' name='voucher_code' value='$code'>";
                        echo "<button type='submit' name='apply_voucher' class='btn btn-outline-primary'>Chọn</button>";
                        echo "</form>";
                        echo "</div></div></div>";
                    }
                    echo "</div>";
            ...
            <?php if(isset($_SESSION['applied_voucher'])): ?>
                <div class="mt-3">
                    <div class="alert alert-info">Mã đang chọn: <strong><?php echo htmlspecialchars($_SESSION['applied_voucher']); ?></strong></div>
                </div>
            <?php endif; ?>
        </div>
