...
checkAdminLogin();
// Chỉ admin (role_level >= 4) trở lên mới được sửa admin
...
<?php
    if(isset($_POST['update_admin'])){
        ...
        // handle image upload: if no new file uploaded, keep existing image
        $uploaded_img = isset($_FILES['admin_img']['name']) ? $_FILES['admin_img']['name'] : '';
        if(!empty($uploaded_img)){
            $temp_name = $_FILES['admin_img']['tmp_name'];
            move_uploaded_file($temp_name, "admin_img/$uploaded_img");
            $admin_img = $uploaded_img;
        } else {
            // $admin_img should already be set from the DB fetch above
            if(!isset($admin_img)) { $admin_img = ''; }
        }
