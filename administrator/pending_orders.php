            ...
            <div class="row">
                ...
                <p>
                    ...
                    <a href="index.php?packing_orders" class = "btn btn-warning text-white me-3">Chờ đóng gói</a>
                ...
                <?php
                    // newest orders first
                    $get_pending_orders = "select * from customer_orders where status = 'Đang chờ' ORDER BY order_id DESC";
...
<?php
    if(isset($_POST['submit_order'])){
        ...
        while($row_order_product = mysqli_fetch_array($run_order_product)){
            ...
            if($product_status=="Ngừng bán"){
                ...
                // Do not directly update product_img here. We'll use auto_export_stock() after confirming the order
                // to create export records and update stock in a single place (avoid double-decrement or missing stock_movements).
            }
        }
        if($count2!=0){
            ...
            if($run_update){
                // After order status updated to 'Đang giao', create stock_movements exports and update quantities centrally
                if (function_exists('auto_export_stock')) {
                    @auto_export_stock($conn, $order_id);
                }
                    // generate a tracking code and store it (ensure column exists)
                    ...
                    // simple tracking code: VN + timestamp + order id
                    ...
                    $msg = "Đơn hàng #$order_id đã được xác nhận và đang giao. Mã vận đơn: $tracking_code";
    ...
    if(isset($_POST['uncorrectinfo'])){
        ...
        if($run_update){
            ...
            $row_c = mysqli_fetch_array($run_get_customer);
            $customer_id = $row_c['customer_id'];
            $msg = "Đơn hàng #$order_id đã bị hủy. Lý do: Thông tin liên hệ không chính xác.";
            $msg_esc = mysqli_real_escape_string($conn, $msg);
            $insert_notify = "insert into notifications (user_id, is_admin, type, title, message, related_id, is_read, created_at) values ('$customer_id', 0, 'order_cancel', 'Đơn bị hủy bởi quản trị', '$msg_esc', '$order_id', 0, NOW())";
            @mysqli_query($conn, $insert_notify);
            echo "<script>window.open('index.php?pending_orders', '_self')</script>";
        }
    }
    ...
    if(isset($_POST['stopsell'])){
        ...
        if($run_update){
            ...
            $row_c = mysqli_fetch_array($run_get_customer);
            $customer_id = $row_c['customer_id'];
            $msg = "Đơn hàng #$order_id đã bị hủy. Lý do: Có sản phẩm trong đơn hàng đã ngừng kinh doanh.";
            $msg_esc = mysqli_real_escape_string($conn, $msg);
            $insert_notify = "insert into notifications (user_id, is_admin, type, title, message, related_id, is_read, created_at) values ('$customer_id', 0, 'order_cancel', 'Đơn bị hủy bởi quản trị', '$msg_esc', '$order_id', 0, NOW())";
            @mysqli_query($conn, $insert_notify);
            echo "<script>window.open('index.php?pending_orders', '_self')</script>";
    ...
    if(isset($_POST['outofstock'])){
        ...
        if($run_update){
            ...
            $row_c = mysqli_fetch_array($run_get_customer);
            $customer_id = $row_c['customer_id'];
            $msg = "Đơn hàng #$order_id đã bị hủy. Lý do: Có sản phẩm trong đơn hàng đã hết hàng.";
            $msg_esc = mysqli_real_escape_string($conn, $msg);
            $insert_notify = "insert into notifications (user_id, is_admin, type, title, message, related_id, is_read, created_at) values ('$customer_id', 0, 'order_cancel', 'Đơn bị hủy bởi quản trị', '$msg_esc', '$order_id', 0, NOW())";
            @mysqli_query($conn, $insert_notify);
            echo "<script>window.open('index.php?pending_orders', '_self')</script>";
    ...
    if(isset($_POST['notenough'])){
        ...
        if($run_update){
            ...
            $row_c = mysqli_fetch_array($run_get_customer);
            $customer_id = $row_c['customer_id'];
            $msg = "Đơn hàng #$order_id đã bị hủy. Lý do: Có sản phẩm không đủ số lượng yêu cầu.";
            $msg_esc = mysqli_real_escape_string($conn, $msg);
            $insert_notify = "insert into notifications (user_id, is_admin, type, title, message, related_id, is_read, created_at) values ('$customer_id', 0, 'order_cancel', 'Đơn bị hủy bởi quản trị', '$msg_esc', '$order_id', 0, NOW())";
            @mysqli_query($conn, $insert_notify);
            echo "<script>window.open('index.php?pending_orders', '_self')</script>";
