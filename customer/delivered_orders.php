                        ...
                        if(isset($_SESSION['customer_id'])){
                            ...
                            while($row_customer_orders = mysqli_fetch_array($run_customer_orders)){
                                ...
                                while($row_customer_order_products = mysqli_fetch_array($run_customer_order_products)){
                                    ...
                                    // color may store either the color name or the product_color_id
                                    if(ctype_digit((string)$color)){
                                        $product_color_id = (int)$color;
                                    } else {
                                        ...
                                        // defensive: ensure color row exists
                                        if($row_color && isset($row_color['product_color_id'])){
                                            $product_color_id = $row_color['product_color_id'];
                                        } else {
                                            $product_color_id = 0; // unknown
                                        }
                                    }
                                    ...
                                    // defensive: product name
                                    if($row_product && isset($row_product['product_name'])){
                                        $product_name = $row_product['product_name'];
                                    } else {
                                        $product_name = 'Sản phẩm';
                                    }
                                    // try to find the product image for the chosen color; if not found, fall back to any image for the product
                                    if($product_color_id){
                                        ...
                                        $row_product_img = false;
                                    }
                                    if(!$row_product_img){
                                        // fallback: any image for the product
                                        $select_product_img = "select * from product_img where product_id = '$product_id' LIMIT 1";
                                        $run_product_img = mysqli_query($conn, $select_product_img);
                                        $row_product_img = mysqli_fetch_array($run_product_img);
                                    }
                                    // defensive: product image and price
                                    if($row_product_img){
                                        $product_img = $row_product_img['product_color_img'] ?? 'placeholder.png';
                                        $product_price_des = $row_product_img['product_price_des'] ?? 0;
                                        $product_price_des_format = currency_format($product_price_des);
                                        $product_price = $row_product_img['product_price'] ?? 0;
                                        $product_price_format = currency_format($product_price);
                                        // if we fell back to a product_img that has a product_color_id, and original $color was numeric 0/unknown, try to get the real color name
                                        if(empty($color) || $product_color_id == 0){
                                            $maybe_color_id = $row_product_img['product_color_id'] ?? 0;
                                            if($maybe_color_id){
                                                $q = "select * from product_color where product_color_id = '$maybe_color_id'";
                                                $r = mysqli_query($conn, $q);
                                                $rc = mysqli_fetch_array($r);
                                                if($rc && isset($rc['product_color_name'])){
                                                    $color = $rc['product_color_name'];
                                                }
                                            }
                                        }
                                    } else {
                                        $product_img = 'placeholder.png';
                                        $product_price_des_format = currency_format(0);
                                        $product_price_format = currency_format(0);
                                    }
                                    echo "
                                        <div class='row mb-4'>
                                            <!-- product img -->
