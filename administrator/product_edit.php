...
?>
            ...
            <style>
                /* Remove rounded corner for product description textarea on this page only */
                #admin-product-edit-page #product_des {
                    border-radius: 0 !important;
                }
            </style>

            <div id="admin-product-edit-page" class="row">
                <div class="col-md-12">
                ...
                <div class="row mb-3">
                    ...
                    <div class="row mb-3 mt-5">
                        <label for=""><strong>Quản lý kho và trạng thái bán</strong></label>
                    </div>
                <table class="table table-striped table-hover">
                    ...
                    <?php
                            ...
                            while($row_get_price = mysqli_fetch_array($run_get_price)){
                                ...
                                // Kiểm tra quyền cao nhất (admin_level >= 4)
                                $admin_level_current = (int)($_SESSION['admin_level'] ?? 0);
                                $can_edit_price = ($admin_level_current >= 4);
                    ?>
                    <tr>
                        <form action="" method="post">
                            ...
                            <td><input type="hidden" name="product_color_img_id" value="<?=$product_color_img_id?>">
                                <button type="submit" name="update_product_stock" class="btn btn-primary btn-sm">Cập nhật</button>
                        </form>
                    </tr>
                    <?php
                        }
                ...
                <!-- PHẦN QUẢN LÝ GIÁ - CHỈ ADMIN CẤP CAO MỚI CÓ QUYỀN -->
                <?php
                    $admin_level_current = (int)($_SESSION['admin_level'] ?? 0);
                    $is_manager = ($admin_level_current >= 4);
                    
                    if($is_manager) {
                        echo '
                        <div class="row mb-3 mt-5">
                            <label for=""><strong><i class="fas fa-dollar-sign"></i> Quản lý giá (Chỉ dành cho Quản lý cấp cao)</strong></label>
                        </div>
                        <table class="table table-striped table-hover">
                        ...
                        while($row_get_price = mysqli_fetch_array($run_get_price)){
                            ...
                            <tr>
                                <form action=\"\" method=\"post\">
                                    <td>$product_color_name</td>
                                    <td><img src=\"product_img/$product_color_img\" width=\"80px\" style=\"object-fit: contain;\" alt=\"\"></td>
                                    <td><input type=\"number\" name=\"product_price\" value=\"$product_price\" class=\"form-control\" required></td>
                                    <td><input type=\"number\" name=\"product_price_des\" value=\"$product_price_des\" class=\"form-control\" required></td>
                                    <td>
                                        <input type=\"hidden\" name=\"product_color_img_id\" value=\"$product_color_img_id\">
                                        <button type=\"submit\" name=\"update_product_price\" class=\"btn btn-warning btn-sm\"><i class=\"fas fa-edit\"></i> Cập nhật giá</button>
                                    </td>
                                </form>
                            </tr>";
                        }
                        ...
                        echo '<div class="alert alert-danger mt-5"><i class="fas fa-lock"></i> <strong>Bạn không có quyền quản lý giá sản phẩm!</strong><br>Chỉ quản lý cấp cao (level >= 4) mới có quyền này. (Level của bạn: ' . htmlspecialchars($admin_level_current) . ')</div>';
                    }
                ?>
                    <?php
                        ...
                        if(isset($_POST['update_product_price'])){
                            // Kiểm tra quyền cao nhất (admin_level >= 4)
                            $admin_level_current = (int)($_SESSION['admin_level'] ?? 0);
                            if($admin_level_current < 4){
                                echo "<script>alert('❌ Bạn không có quyền sửa giá sản phẩm! Chỉ quản lý cấp cao (level >= 4) mới có quyền này. (Level của bạn: " . htmlspecialchars($admin_level_current) . ")')</script>";
                            } else {
                                ...
                                if($product_quantity1 < 0 || $product_price1 < 0 || $product_price_des1 < 0){
                                    ...
                                    // Lấy giá cũ để log lịch sử
                                    $get_old_price = "SELECT product_price, product_price_des FROM product_img WHERE product_color_img_id='$product_color_img_id1'";
                                    $res_old = mysqli_query($conn, $get_old_price);
                                    $old_row = mysqli_fetch_assoc($res_old);
                                    $old_price = (int)$old_row['product_price'];
                                    $old_discount_price = (int)$old_row['product_price_des'];
                                    ...
                                    if($run_product_price){
                                        ...
                                        log_price_change($conn, $product_color_img_id1, $old_price, $product_price1, $old_discount_price, $product_price_des1, 'Cập nhật từ admin');
                ...
                <?php
                    // Xử lý cập nhật kho và trạng thái bán
                    if(isset($_POST['update_product_stock'])){
                        ...
                        if($product_quantity1 < 0){
                            echo "<script>alert('❌ Số lượng sản phẩm phải >= 0')</script>";
                        } else {
                            $update_stock = "UPDATE product_img SET product_quantity='$product_quantity1', product_status='$product_status1' WHERE product_color_img_id='$product_color_img_id1'";
                            $run_update_stock = mysqli_query($conn, $update_stock);
                            
                            if($run_update_stock){
                                echo "<script>alert('✅ Cập nhật kho thành công')</script>";
                                echo "<script>window.open('index.php?product_edit=$product_id','_self')</script>";
                            } else {
